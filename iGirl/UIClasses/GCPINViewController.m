...
@synthesize messageText2 = __messageText2;
...
- (void)presentFromViewController:(UIViewController *)controller animated:(BOOL)animated {
    ...
    [navController setValue:customNavigationBar forKeyPath:@"navigationBar"];
...
- (void)viewDidLoad {
    ...
    if (self.mode == GCPINViewControllerModeCreate) {
        ...
        [cancelButton setBackgroundImage:[[UIImage imageNamed:@"navigationBarButton"] stretchableImageWithLeftCapWidth:20 topCapHeight:16] forState:UIControlStateNormal];
        [cancelButton setBackgroundImage:[[UIImage imageNamed:@"navigationBarButton_selected"] stretchableImageWithLeftCapWidth:20 topCapHeight:16] forState:UIControlStateHighlighted];
        ...
        [customNavigationBar setText:NSLocalizedString(@"取消", @"") onBackButton:cancelButton leftCapWidth:14.0];
...
- (void)setMessageText2:(NSString *)text {
    __messageText2 = [text copy];
}
        ...
        if ([self.inputField.text length] == 4) {
            if (self.mode == GCPINViewControllerModeCreate) {
                if (self.text == nil) {
                    ...
                    self.messageLabel.text = __messageText2;
                }
                else {
